...
struct RenameFixture
{
    ...
    RenameFixture()
    {
        ...
        tabs.tabs().push_back(make_test_tab(1, "alpha"));
        tabs.tabs().push_back(make_test_tab(2, "beta"));
        tabs.activate_tab(1);
...
TEST_CASE("ChromeHost rename: typing and Enter commits", "[chrome_host][rename]")
{
    ...
    REQUIRE(f.space_controller.active_tab_controller().tabs()[0]->name == "alphadev");
    REQUIRE(f.space_controller.active_tab_controller().tabs()[0]->name_user_set);
    REQUIRE(f.space_controller.active_tab_controller().tabs()[1]->name == "beta");
    REQUIRE_FALSE(f.space_controller.active_tab_controller().tabs()[1]->name_user_set);
}
...
TEST_CASE("ChromeHost rename: Backspace deletes the previous codepoint", "[chrome_host][rename]")
{
    ...
    REQUIRE(f.host->on_rename_key(SDLK_BACKSPACE));
    REQUIRE(f.host->on_rename_key(SDLK_RETURN));
    REQUIRE(f.space_controller.active_tab_controller().tabs()[0]->name == "alp");
}
...
TEST_CASE("ChromeHost rename: empty buffer commit keeps the original name", "[chrome_host][rename]")
{
    ...
    REQUIRE(f.space_controller.active_tab_controller().tabs()[0]->name == "alpha");
    REQUIRE_FALSE(f.space_controller.active_tab_controller().tabs()[0]->name_user_set);
}
    ...
    "[chrome_host][rename]")
{
    ...
    REQUIRE(f.space_controller.active_tab_controller().tabs()[0]->name == "alphaX");
    REQUIRE(f.space_controller.active_tab_controller().tabs()[0]->name_user_set);
}
