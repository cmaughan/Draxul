...
TEST_CASE("mpack fuzz: empty buffer (0 bytes)", "[rpc][fuzz]")
{
    assert_safe_decode({}, "empty buffer");
}

TEST_CASE("mpack fuzz: single-byte buffers (all 256 values)", "[rpc][fuzz]")
{
    for (int b = 0; b < 256; ++b)
    {
...
TEST_CASE("mpack fuzz: fixarray header truncated body", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: fixmap header truncated body", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: str8 header truncated body", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: bin8 header truncated body", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: array16 header truncated body", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: uint64 header truncated body", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: deeply nested arrays (10 levels)", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: deeply nested arrays (50 levels)", "[rpc][fuzz]")
{
    std::vector<uint8_t> buf;
    for (int i = 0; i < 50; ++i)
        buf.push_back(0x91);
...
TEST_CASE("mpack fuzz: deeply nested arrays (50 levels) truncated at leaf", "[rpc][fuzz]")
{
    std::vector<uint8_t> buf;
    for (int i = 0; i < 50; ++i)
        buf.push_back(0x91);
...
TEST_CASE("mpack fuzz: ext type byte 0 (neovim buffer handle)", "[rpc][fuzz]")
{
    const uint8_t buf[] = { 0xd4, 0x00, 0x01 };
    assert_safe_decode(buf, "ext type 0");
}

TEST_CASE("mpack fuzz: ext type byte 127 (max positive)", "[rpc][fuzz]")
{
    const uint8_t buf[] = { 0xd4, 0x7f, 0xff };
    assert_safe_decode(buf, "ext type 127");
}

TEST_CASE("mpack fuzz: ext type byte -1 (0xff signed)", "[rpc][fuzz]")
{
    const uint8_t buf[] = { 0xd4, 0xff, 0x00 };
    assert_safe_decode(buf, "ext type -1");
}

TEST_CASE("mpack fuzz: ext type byte -128 (0x80 signed, min int8)", "[rpc][fuzz]")
{
    const uint8_t buf[] = { 0xd4, 0x80, 0x00 };
    assert_safe_decode(buf, "ext type -128");
}

TEST_CASE("mpack fuzz: fixext4 with unexpected type byte", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: ext8 with large payload (>8 bytes — skipped path)", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: ext8 header present but payload truncated", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: fixstr with overlong UTF-8 sequence", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: fixstr with lone continuation bytes", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: fixstr with truncated multi-byte sequence", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: valid value followed by garbage bytes", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: fixmap(2) with only one complete pair", "[rpc][fuzz]")
{
...
TEST_CASE("mpack fuzz: nested array inside map, leaf truncated", "[rpc][fuzz]")
{
