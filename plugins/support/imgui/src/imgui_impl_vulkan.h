...
// Retires the current plugin-owned atlas without destroying resources that may
// still be referenced by Draxul's in-flight frames. A subsequent NewFrame
// records the replacement upload into the borrowed command buffer.
IMGUI_IMPL_API bool         ImGui_ImplVulkan_RetireFontsTexture();
// ScoreView records font uploads into Draxul's borrowed command buffer. The
// backend must never allocate, submit, or wait on its own command buffer.
IMGUI_IMPL_API void         ImGui_ImplVulkan_SetExternalCommandBuffer(VkCommandBuffer command_buffer);
