...
struct ImGui_ImplVulkan_Data
{
    ...
    VkBuffer                    DeferredFontUploadBuffers[64];
    VkDeviceMemory              DeferredFontUploadMemory[64];
    uint32_t                    DeferredFontUploadCount;
    VkDeviceMemory              RetiredFontMemory[64];
    VkImage                     RetiredFontImage[64];
    VkImageView                 RetiredFontView[64];
    VkDescriptorSet             RetiredFontDescriptorSet[64];
    uint32_t                    RetiredFontCount;
...
bool ImGui_ImplVulkan_CreateFontsTexture()
{
    ...
    if (bd->ExternalCommandBuffer == VK_NULL_HANDLE)
        return false;
    const VkCommandBuffer command_buffer = bd->ExternalCommandBuffer;
    ...
    // Draxul owns submission. Retain upload storage until plugin destruction,
    // which occurs only after the renderer has gone idle.
    if (bd->DeferredFontUploadCount < IM_ARRAYSIZE(bd->DeferredFontUploadBuffers))
    {
        const uint32_t index = bd->DeferredFontUploadCount++;
        bd->DeferredFontUploadBuffers[index] = upload_buffer;
        bd->DeferredFontUploadMemory[index] = upload_buffer_memory;
    }

    return true;
}

void ImGui_ImplVulkan_SetExternalCommandBuffer(VkCommandBuffer command_buffer)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (bd != nullptr)
        bd->ExternalCommandBuffer = command_buffer;
}

bool ImGui_ImplVulkan_RetireFontsTexture()
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (!bd || (!bd->FontDescriptorSet && !bd->FontView
            && !bd->FontImage && !bd->FontMemory))
        return true;
    if (bd->RetiredFontCount >= IM_ARRAYSIZE(bd->RetiredFontImage))
        return false;
    const uint32_t index = bd->RetiredFontCount++;
    bd->RetiredFontMemory[index] = bd->FontMemory;
    bd->RetiredFontImage[index] = bd->FontImage;
    bd->RetiredFontView[index] = bd->FontView;
    bd->RetiredFontDescriptorSet[index] = bd->FontDescriptorSet;
...
void    ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ...
    for (uint32_t index = 0; index < bd->DeferredFontUploadCount; ++index)
    {
        if (bd->DeferredFontUploadBuffers[index])
            vkDestroyBuffer(v->Device, bd->DeferredFontUploadBuffers[index], v->Allocator);
        if (bd->DeferredFontUploadMemory[index])
            vkFreeMemory(v->Device, bd->DeferredFontUploadMemory[index], v->Allocator);
    }
    bd->DeferredFontUploadCount = 0;

    for (uint32_t index = 0; index < bd->RetiredFontCount; ++index)
    {
        if (bd->RetiredFontDescriptorSet[index])
            ImGui_ImplVulkan_RemoveTexture(
                bd->RetiredFontDescriptorSet[index]);
        if (bd->RetiredFontView[index])
            vkDestroyImageView(v->Device, bd->RetiredFontView[index], v->Allocator);
        if (bd->RetiredFontImage[index])
            vkDestroyImage(v->Device, bd->RetiredFontImage[index], v->Allocator);
        if (bd->RetiredFontMemory[index])
            vkFreeMemory(v->Device, bd->RetiredFontMemory[index], v->Allocator);
    }
    bd->RetiredFontCount = 0;
