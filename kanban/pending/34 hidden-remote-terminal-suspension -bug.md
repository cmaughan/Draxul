...
- [x] Server terminal runtimes continue pumping output at their normal cadence with
...
- [x] Topology and agent projection polling remains independent and active for an
      attached UI, so remote pane/tab mutations and agent attention still converge
      while terminal presentation is suspended.
- [x] Suspending presentation does not disconnect the UI client, release its terminal
      controller claim, restart the process, change terminal identity/generation, or
      clear server scrollback.
- [x] Resuming is atomic from the client's perspective: it reactivates delivery and
...
- [x] A client that actually disappears still expires through the existing client
      activity lease and releases subscriptions and controller claims normally.
- [x] Pending input, paste, resize, restart, scrollback, and take-control work is never
...
- [x] Add explicit idempotent terminal presentation suspend/resume operations rather
...
- [x] Model subscriber delivery as active or suspended. A suspended subscriber retains
      its client/controller identity but owns no delta queue and is marked as requiring
      a complete snapshot when resumed.
- [x] Do not encode a terminal delta when a terminal has no active presentation
      subscribers. With mixed active and suspended clients, encode once and queue only
      for active subscribers.
- [x] Resume and snapshot publication happen on the server state thread so output cannot
      slip between reactivation and the returned authoritative version.
- [x] Keep the existing 32-event/2-MiB slow-subscriber resync behavior for active
      subscribers; suspension must not consume those queue limits.
- [x] Extend sanitized `terminal.metrics` with active/suspended subscriber counts,
...
- [x] OSC 52 clipboard writes produced while the controlling UI is suspended do not
...
- [x] Process exit, controller identity, title, cwd, cursor, and mode state converge via
...
- [x] Preserve the last rendered client grid while suspended, but clear or revalidate
...
- [x] Add an explicit presentation-visibility lifecycle hook to `IHost`, defaulting to
      a no-op. `RemoteTerminalHost` implements it without platform-specific casts in
      `App` or `PaneManager`.
- [x] Mark all panes in the active tab of the active Space visible; mark panes in other
      tabs and Spaces hidden. Keep zoom/minimize policy unchanged in the first slice so
      the visibility boundary is deterministic.
- [x] Implement a nonblocking remote-host worker state machine:
      `Active -> Suspending -> Suspended -> Resuming -> Active`.
- [x] Drain or safely retain already queued commands before entering `Suspended`. A
      visibility or command wake transitions immediately to `Resuming`.
- [x] While suspended, schedule no terminal polls or 25-ms host deadline, publish no
      grid state, wake no window for terminal output, flush no hidden grid, and request
      no frame.
- [x] On resume, asynchronously obtain the authoritative snapshot, reconcile controller
...
- [x] Ignore stale worker publications from a pre-suspend visibility generation so an
      in-flight poll cannot wake or repaint the newly hidden tab.
- [x] Keep client-local Nvim and other hosts on their existing background-pump policy;
      this change applies only to server-owned remote-terminal presentation.

## Integration-first validation

- [x] Server/client integration: attach as controller, suspend, produce more than 32
...
- [x] Two-client integration: suspend the controller while an observer remains active;
      prove the observer continues receiving deltas and explicit takeover still works.
- [x] Lease integration: suspend a client, stop its Session heartbeat, advance beyond
      the activity timeout, and prove normal detachment/controller release.
- [x] Recovery integration: restart the server or replace the terminal generation while
      hidden; activation reattaches to the new epoch/generation without reviving stale
      cells or commands.
- [x] Ephemeral integration: clipboard writes while suspended follow the documented
      suppression policy; title/cwd/process/controller state appears after resume.
- [x] App vertical slice: run an active interactive terminal beside an inactive tab of
...
- [x] Resize/scrollback integration: resize the window while a tab is hidden, then
      activate it and prove current dimensions, live viewport, and retained server
      scrollback are coherent.
- [x] Prefer the existing fake/isolated server integration fixtures and smoke path.
...
- [x] With the ten-pane dashboard tab hidden for at least ten seconds, its terminal
