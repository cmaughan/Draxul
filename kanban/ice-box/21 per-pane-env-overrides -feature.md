...
- [ ] Add shell overrides to the versioned server launch descriptor and apply
      them in `draxul-terminal-process`. Keep Nvim overrides on its client-owned
      process path.
- [ ] In the server process spawn paths (`fork()`/`exec()` on Unix,
      `CreateProcess` on Windows), apply overrides on top of the inherited
