...
Protect important long-running panes or complete workspaces from accidental close, restart, replacement, or destructive layout actions.

## Implementation plan

- [ ] Add `locked` metadata to pane/workspace controller records and versioned session state; locking a workspace protects all current panes plus workspace deletion.
- [ ] Centralize destructive-operation authorization in `WorkspaceController`/HostManager so palette, keybinding, window close, layout template, move, and session actions cannot bypass it.
...
- [ ] Exercise every destructive entry point against pane lock, workspace lock, both, force, unlock, restore, and corrupted metadata.
