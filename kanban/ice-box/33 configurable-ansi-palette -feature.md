...
The ANSI 16-colour palette is resolved in terminal-core processing. Add a
`[terminal] ansi_colors = [...]` array while preserving the server/client
boundary: palette selection is presentation state and may differ between two
...
### Files to modify
- terminal protocol/core — preserve ANSI palette indices as semantic attributes
...
- [ ] Extend the versioned attribute protocol if necessary so ANSI indices are
  not irreversibly converted to RGB by the server.
- [ ] Resolve indices against the client palette in remote presentation.
