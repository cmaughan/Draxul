...
- [x] After **WI 115** lands: `grep -n 'R"(' libs/draxul-host/src/nvim_host.cpp` — list all raw string literals
- [x] Categorise each: which are Lua? Any other embedded scripts?
- [x] Count total Lua blocks to estimate scope

---

## Acceptance Criteria

- [x] No multi-line Lua literals in `nvim_host.cpp`
- [x] Each extracted script is named meaningfully
...
Implemented the minimum viable Option B. The Lua snippets now live in `libs/draxul-host/src/nvim_lua_scripts.h` as named constants, and `nvim_host.cpp` references those constants from the action handlers and clipboard provider setup.
