# Publish HDR targets only after complete initialization

**Severity:** CRITICAL  
**Reported by:** Claude Fable 5.1

`plugins/satview/src/render/satview_render_vk.cpp:1048` considers matching vector size and first-target dimensions sufficient. A later failure at `:1076`, `:1094`, or `:1107` leaves that condition true, allowing subsequent frames to use incomplete targets.

**Investigation**

- [x] Inject failures after the first target succeeds and within later attachment/descriptor creation.

**Fix strategy**

- [x] Build a temporary complete target set before publication, or clear every partial set on failure.
- [x] Make validity checks cover all required frame resources.

**Acceptance criteria**

- [x] Failed creation cannot lead to null framebuffer or descriptor use.
- [x] A subsequent successful retry recovers cleanly without leaks.
- [x] Preserve Metal failure behavior.

**Validation**

- [x] macOS: `python3 do.py test debug --satview` (26/26 CTest entries) and same-cache `python3 do.py smoke debug --skip-build` pass.
- [x] Build the Vulkan renderer and run the SatView suite on Windows; macOS compiles the unchanged Metal backend.

## Windows closeout (2026-09-22)

The Vulkan-backed SatView suite passed in the final 48/48 products aggregate,
an additional SatView scenario exported a valid 960x640 frame, and smoke passed.
