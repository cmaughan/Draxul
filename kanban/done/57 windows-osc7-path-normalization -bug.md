# Normalize Windows OSC 7 paths before reuse

**Severity:** HIGH  
**Reported by:** Claude Fable 5.1

`libs/draxul-terminal-core/src/terminal_core_csi.cpp:689` retains the URI’s leading slash for Windows drive paths. Metadata is reused as a working directory, while `app/app.cpp:3177` recognizes only `/` when naming tabs. Windows OSC 7 paths can therefore produce invalid launch directories or incorrect names.

**Investigation**

- [x] Trace drive, UNC, percent-encoded, and backslash-containing paths through metadata, naming, and pane launch.

**Fix strategy**

- [x] Convert file URIs to platform-appropriate filesystem paths.
- [x] Extract display basenames using platform-aware rules while preserving POSIX behavior.

**Acceptance criteria**

- [x] Windows drive and supported UNC paths round-trip into valid working directories.
- [x] Tab names show the intended basename.
- [x] POSIX paths remain unchanged semantically.

## Windows closeout (2026-09-22)

Drive, UNC, basename, and preserved POSIX-semantics regressions passed in the
final 48/48 products aggregate; same-cache smoke passed.
