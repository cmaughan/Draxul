...
- [x] Read `libs/draxul-host/src/nvim_host.cpp` around line 101 to understand what triggers the poll and what it does with the result (likely updating a cached value for the `+` and `*` Neovim registers).
- [x] Read `libs/draxul-window/include/draxul/window.h` and `libs/draxul-window/src/sdl_window.cpp` to understand whether `clipboard_text()` already caches at the SDL level.
- [x] Check whether `SdlWindow` emits a `ClipboardUpdateEvent` (SDL3 has `SDL_EVENT_CLIPBOARD_UPDATE`) that could be used instead of polling.
...
- [x] `libs/draxul-host/src/nvim_host.cpp` — remove per-frame poll; subscribe to `ClipboardUpdateEvent` or implement lazy-pull cache.
- [x] `libs/draxul-host/include/draxul/nvim.h` — update `NvimHost` event callback signature if needed.
...
- [x] `NvimHost::pump()` no longer calls `clipboard_text()` on every frame.
...
Implemented Option B inside NvimHost ownership: `pump()` now checks a `ClipboardPollGate` and refreshes the main-thread clipboard cache only after the configured interval. Startup still seeds the cache, Draxul clipboard writes update it immediately, and the reader-thread RPC handler continues to read only the cache.

Full target builds are currently blocked by unrelated in-flight config edits (`AppConfig`/`app_config_types.h`). Direct C++20 syntax checks passed for `libs/draxul-host/src/nvim_host.cpp` and `tests/clipboard_poll_gate_tests.cpp`.
