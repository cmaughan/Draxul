...
- [x] Read `app/app.cpp:30-90` (RenderTestPhase / RenderTestContext / phase-name helper) and `app.cpp:795-975` (`run_render_test`).
- [x] Read `app/app.cpp:762-793` (`run_screenshot`) — decide whether to bring it into scope.
- [x] Read `libs/draxul-render-test/include/draxul/render_test.h` and `src/render_test.cpp` to confirm the existing seam.
- [x] Check `app/main.cpp:84-130, 292-396` to see how render-test flags flow from CLI → `AppOptions` → `App::run_render_test()` → finalize/export.
- [x] Confirm there are no existing unit tests for the render-test state machine (`tests/` does not appear to cover it).
...
- [x] Add `libs/draxul-render-test/include/draxul/render_test_driver.h` and `src/render_test_driver.cpp`. Wire into `libs/draxul-render-test/CMakeLists.txt`.
- [x] Move `RenderTestPhase`, `RenderTestContext`, `render_test_phase_name`, and the body of `App::run_render_test` (the state machine + timeout-error formatter) into `render_test_driver.cpp`.
- [x] Replace `App::run_render_test()` with a thin shim that constructs `RenderTestDriverEnv`, calls `run_render_test_driver`, and stores the error string.
- [x] Confirm `app/main.cpp` still compiles unchanged — the render-test CLI flow only depends on `App::run_render_test()`'s public signature, which is preserved.
- [x] Add unit tests in `tests/render_test_driver_tests.cpp` covering at least:
...
- [x] Run `cmake --build build --target draxul draxul-tests` and `ctest --test-dir build -R draxul-tests`.
- [x] Run `py do.py smoke` to confirm the app still launches.
...
- [x] `app/app.cpp` no longer contains `RenderTestPhase`, `RenderTestContext`, `render_test_phase_name`, or the render-test state-machine body.
- [x] `App::run_render_test()` is a thin shim (~30 lines) that delegates to `run_render_test_driver`.
- [x] `last_render_test_error()` continues to return the same error strings (verified by an existing failing scenario or a unit test).
- [x] New unit tests cover the four cases above and pass in the normal test target.
- [ ] All existing render-test scenarios (`py do.py blessall` outputs match) pass unchanged.
- [x] `py do.py smoke` passes.

## Completion Notes

Added `RenderTestDriverEnv`, `RenderTestDriverOptions`, and `run_render_test_driver()` in `libs/draxul-render-test`; `App::run_render_test()` now only adapts app state to the driver and stores the returned error. Focused unit tests cover no-diagnostics success, diagnostics success, no-host timeout, and capture timeout. `run_screenshot()` and AppOptions cleanup were left as optional follow-ups.
