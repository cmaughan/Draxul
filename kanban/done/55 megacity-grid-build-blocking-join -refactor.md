...
- [x] Read `libs/draxul-megacity/src/megacity_host.cpp` — understand the full lifecycle of `grid_thread_` and `launch_grid_build()`: how it is started, how the thread signals completion, and where `join()` is called.
- [x] Read `libs/draxul-megacity/include/draxul/megacity_host.h` — check the data members used by the build thread.
- [x] Design a cancellation approach: add an `std::atomic<bool> cancel_build_` flag that the build thread checks periodically. When a new build is requested while one is running: set the cancel flag, detach or let the old thread self-terminate, then start the new one.
- [x] If the build thread holds shared state that is unsafe to abandon: use a generation counter (`std::atomic<int> build_gen_`) so the thread's result is discarded if the generation no longer matches when it tries to commit.
- [x] Remove the synchronous `join()` from any path called from `pump()`. A blocking join on destruction (in the destructor) is acceptable.
- [x] Ensure the destructor still joins cleanly so the thread does not outlive the host.
- [x] Build: `cmake --build build --target draxul draxul-tests`
- [x] Run smoke test and MegaCity test if available: `py do.py smoke`

Implementation note: added a focused MegaCity regression test for non-blocking rebuild requests. Integrated build, CTest, smoke, and graphify validation now pass.
