...
- [x] A `[chrome]` table in `config.toml` with any subset of keys overrides ChromeHost colors.
- [x] Omitting `[chrome]` entirely preserves current Catppuccin Mocha defaults.
- [x] Unknown keys under `[chrome]` produce a WARN log.
- [x] `docs/features.md` documents all new config keys.
