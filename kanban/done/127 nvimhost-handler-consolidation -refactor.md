...
- [x] After **WI 115** lands, audit `NvimHost::dispatch_action()` for all handlers
- [x] Identify which are nvim-specific vs. generic (generic ones should probably be in a base class or `HostManager`)
- [x] Check if action name strings are typed/validated elsewhere (link to `15 guiactionhandler-dispatch-map`)

---

## Acceptance Criteria

- [x] `dispatch_action()` contains no if/else chain — only a dispatch-table lookup
- [x] Action names appear in exactly one place (the table)
...
`NvimHost::dispatch_action()` now uses exact and prefixed dispatch tables. Each action routes to a small private handler method, and the tables are checked with a compile-time duplicate-name assertion.

Full target builds/tests are currently blocked by unrelated in-flight config edits (`AppConfig`/`app_config_types.h`). Direct C++20 syntax checks passed for `libs/draxul-host/src/nvim_host.cpp`.
