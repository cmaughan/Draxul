...
constexpr float kPerspectiveCascadeLambda = 0.65f;
...
float perspective_near_plane(const glm::mat4& proj)
{
    const float a = proj[2][2];
    const float b = proj[3][2];
    if (std::abs(a) <= 1e-6f)
        return 0.1f;
    return std::max(0.1f, b / a);
}

float perspective_far_plane(const glm::mat4& proj)
{
    const float a = proj[2][2];
    const float b = proj[3][2];
    if (std::abs(a + 1.0f) <= 1e-6f)
        return 100.0f;
    return std::max(perspective_near_plane(proj) + 1.0f, b / (a + 1.0f));
}

float perspective_depth_to_ndc(const glm::mat4& proj, float view_depth)
{
    const float a = proj[2][2];
    const float b = proj[3][2];
...
std::array<float, kShadowCascadeCount> cascade_split_depths(const CodeVizCameraData& camera)
{
    ...
    const float near_plane = perspective_near_plane(camera.proj);
    const float far_plane = perspective_far_plane(camera.proj);
    const float depth_ratio = std::max(far_plane / std::max(near_plane, 1e-3f), 1.0f);
    for (size_t cascade_index = 0; cascade_index < split_depths.size(); ++cascade_index)
    {
        const float t = static_cast<float>(cascade_index + 1) / static_cast<float>(split_depths.size());
        const float uniform_split = near_plane + (far_plane - near_plane) * t;
        const float logarithmic_split = near_plane * std::pow(depth_ratio, t);
        const float split_depth = std::lerp(uniform_split, logarithmic_split, kPerspectiveCascadeLambda);
        split_depths[cascade_index] = perspective_depth_to_ndc(camera.proj, split_depth);
    }
    split_depths.back() = 1.0f;
    return split_depths;
}
    ...
    int resolution)
{
    ...
    const glm::vec3 light_eye = center - light_forward * (radius + 2.0f);
...
DirectionalShadowCascadeSet build_directional_shadow_cascades(const CodeVizCameraData& camera, int resolution)
{
    ...
    // TODO: scene_far_ndc clamping disabled — caused shimmer when camera moves.
    // const float scene_depth = std::clamp(camera.scene_far_ndc, 0.1f, 1.0f);

    const auto split_depths = cascade_split_depths(camera);
    float split_start = 0.0f;
    for (size_t cascade_index = 0; cascade_index < cascade_set.cascades.size(); ++cascade_index)
    {
        const float split_end = split_depths[cascade_index];
