...
bool render_renderer_controls(MegacityRendererControls& controls)
{
    ...
    {
        static constexpr const char* kProjectionModeLabels[] = {
            "Orthographic",
            "Perspective",
        };
        int projection_mode = static_cast<int>(config.projection_mode);
        ImGui::SetNextItemWidth(150.0f);
        if (ImGui::Combo("Projection", &projection_mode, kProjectionModeLabels, IM_ARRAYSIZE(kProjectionModeLabels)))
        {
            config.projection_mode = static_cast<MegaCityProjectionMode>(projection_mode);
            changed = true;
    ...
    if (ImGui::TreeNodeEx("##renderer_build", ImGuiTreeNodeFlags_SpanAvailWidth, "City Build"))
    {
        ...
        if (ImGui::TreeNodeEx("##build_module", ImGuiTreeNodeFlags_SpanAvailWidth | ImGuiTreeNodeFlags_DefaultOpen, "Module / View"))
        {
            ...
            if (hide_structs_changed)
                controls.committed_edit = true;
            if (!config.hide_struct_entities)
            {
                const bool stack_changed = ImGui::Checkbox("Stack Structs", &config.enable_struct_stacking);
                changed |= stack_changed;
                if (stack_changed)
                    controls.committed_edit = true;
                if (config.enable_struct_stacking)
                {
                    if (ImGui::SliderInt("Stack Floors", &config.struct_stack_max, 1, 20))
                    {
                        changed = true;
                        controls.committed_edit = true;
                    }
                    edit_float("Struct Stack Gap", config.struct_stack_gap, 0.01f, 0.0f, 1.0f, "%.2f");
                    if (ImGui::SliderInt("Brick Grid Size", &config.struct_brick_grid_size, 1, 10))
                    {
                        changed = true;
                        controls.committed_edit = true;
                    }
            ...
            const bool hide_functions_changed = ImGui::Checkbox("Hide Function Entities", &config.hide_function_entities);
            changed |= hide_functions_changed;
            if (hide_functions_changed)
                controls.committed_edit = true;
            if (ImGui::SliderInt("Functions Per Building", &config.functions_per_building_max, 1, 500))
            {
                changed = true;
                controls.committed_edit = true;
            }
    ...
    if (ImGui::TreeNodeEx("##renderer_signs", ImGuiTreeNodeFlags_SpanAvailWidth, "Signs"))
    {
        ...
        if (ImGui::TreeNodeEx("##signs_colors", ImGuiTreeNodeFlags_SpanAvailWidth, "Colors"))
        {
            ...
            edit_color3("Function Board", config.function_sign_board_color);
            edit_color3("Function Text", config.function_sign_text_color);
            edit_color3("Struct Board", config.struct_sign_board_color);
            edit_color3("Struct Text", config.struct_sign_text_color);
            ImGui::TreePop();
        }
