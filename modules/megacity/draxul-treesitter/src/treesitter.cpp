...
bool should_skip_path(const std::filesystem::path& rel)
{
    ...
    for (const auto& part : rel)
    {
        ...
        if (!s.empty() && s[0] == '.')
            return true;
        if (s == "build" || s == "_deps" || s == "CMakeFiles" || s == "vcpkg")
            return true;
    }
    return false;
}
...
TSNode find_descendant_of_type(TSNode node, const char* type)
{
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        if (node_type_is(child, type))
            return child;
        const TSNode found = find_descendant_of_type(child, type);
...
std::vector<SymbolRecord::FieldRecord> collect_parameter_records(TSNode function_node, const std::string& source)
{
    PERF_MEASURE();
    std::vector<SymbolRecord::FieldRecord> fields;
    const TSNode param_list = find_descendant_of_type(function_node, "parameter_list");
    if (ts_node_is_null(param_list))
        return fields;

    const uint32_t child_count = ts_node_named_child_count(param_list);
    for (uint32_t i = 0; i < child_count; ++i)
    {
        const TSNode param = ts_node_named_child(param_list, i);
        if (!node_type_is(param, "parameter_declaration")
            && !node_type_is(param, "optional_parameter_declaration"))
            continue;

        std::set<std::string> refs;
        collect_type_references(param, source, refs);
        if (refs.empty())
            continue;

        const std::string type_name = field_display_type_from_declaration(param, source);

        // Try to find the parameter name (identifier or declarator).
        std::string param_name;
        const uint32_t param_child_count = ts_node_named_child_count(param);
        for (uint32_t j = 0; j < param_child_count; ++j)
        {
            const TSNode pchild = ts_node_named_child(param, j);
            if (node_type_is(pchild, "identifier") || node_type_is(pchild, "reference_declarator")
                || node_type_is(pchild, "pointer_declarator"))
            {
                param_name = node_text(source, pchild);
                break;
            }
        }
        if (param_name.empty())
        ...
        const std::vector<std::string> referenced_types(refs.begin(), refs.end());
        fields.push_back({ param_name, type_name, referenced_types });
    }
    return fields;
}
