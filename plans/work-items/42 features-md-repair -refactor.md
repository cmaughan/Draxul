...
- [ ] `docs/features.md` accurately reflects the current feature set as implemented in the
      codebase at the time of this fix.
- [ ] The diagnostics section correctly describes the diagnostics panel (F12, ImGui overlay,
      per-step startup timings, frame stats, atlas usage).
- [ ] The keybindings section accurately describes all configurable GUI keybinding actions
      (`toggle_diagnostics`, `copy`, `paste`, `font_increase`, `font_decrease`, `font_reset`,
      and any others that exist).
- [ ] The config tables accurately reflect `config.toml` sections and keys.
- [ ] After the repair, the document is parseable as valid Markdown (no broken tables,
