...
PluginHost::PluginHost(std::shared_ptr<PluginManager> manager,
    ...
    , storage_(std::make_unique<PluginStorage>(std::move(storage_root)))
{
}
...
bool PluginHost::initialize(const HostContext& context, IHostCallbacks& callbacks)
{
    ...
    display_ppi_ = context.display_ppi > 0.0f
        ? context.display_ppi
        : 96.0f * std::max(0.01f, viewport_.pixel_scale);
    ...
    std::string& warning, std::string& error, bool defer_storage_commit)
{
    ...
    if (start_instance(candidate, candidate_error))
    {
        ...
        if (!defer_storage_commit
            && !storage_->commit_overlay(storage_error))
        {
            if (!warning.empty())
                warning += " ";
...
bool PluginHost::finalize_reload_storage(std::string& error)
{
    return storage_->commit_overlay(error);
}
...
void PluginHost::set_imgui_font(const std::string& path,
    float size_pixels)
{
    if (imgui_font_path_ == path && imgui_font_size_pixels_ == size_pixels)
        return;
    imgui_font_path_ = path;
    imgui_font_size_pixels_ = size_pixels;
    ++ui_style_generation_;
    ...
    void* service_table, size_t service_table_size)
{
    ...
    if (id == DRAXUL_PLUGIN_UI_STYLE_SERVICE_ID
        && requested_version == DRAXUL_PLUGIN_UI_STYLE_SERVICE_VERSION
        && service_table_size >= sizeof(DraxulPluginUiStyleServiceV2))
    {
...
int32_t PluginHost::get_recommended_font(void* context, char* path,
    ...
    uint64_t* generation)
{
    ...
    *display_scale = std::max(0.01f, host->viewport_.pixel_scale);
    *generation = host->ui_style_generation_;
    ...
    char* buffer, size_t* in_out_size)
{
    auto* host = callback_host(context);
    return host
        ? host->storage_->get_service_path(path_kind, buffer, in_out_size)
        : 0;
}
