...
- **SatView interaction panel**: SatView owns a pane-local ImGui panel toggled by `F1`, with pause/resume, reset camera, catalog refresh, time-speed, color mode (`Name Prefix`, `Orbit Class`, or `Object Type`), path display (`All Sampled` or `Selected Only`), track/marker LOD controls, search, orbit-class, optional object type/classification, source-label, and element-age filters. Name-prefix mode uses the same stable bright pastel for every object grouped under a tree prefix, so constellations such as STARLINK remain visually consistent. Click selection projects visible satellite markers to screen space, while the object tree groups all propagated objects by orbit class and name prefix for direct selection. Selected objects remain force-visible, highlight in the dynamic line stream, sample their path even when outside the general path cap, and report details including NORAD id, international designator, orbit class, normalized object kind, period, element age, altitude, and speed.
...
- **SatView camera**: Left-drag orbits and `Ctrl` + left-drag/mouse wheel dollies; `A/D`, left/right arrows, or `Q/E` orbit horizontally, `W/S`, up/down arrows, or `T/G` orbit vertically, and `R/F` dolly in/out
