...
- `launch_agent` is available in the command palette and unbound by default. It opens a profile picker with built-in Codex and Claude entries plus structured `[agents.profiles.<id>]` configuration. Managed agents launch directly as the PTY/ConPTY child using executable-plus-argv data, receive Draxul Session/Space/tab/pane/instance routing environment variables, and record durable kind/display/instance identity on the pane. Legacy saved shell-command agents retain their compatibility path. Session restore starts the saved executable as a fresh process; native conversation resume is deliberately not implied yet.
...
- Agent runtime state, semantic observations, explanations, attention latches, and sidebar rows are projections, not persisted state. Only the optional pane-owned agent identity is stored, so live status cannot become stale serialized data and terminal evidence cannot leak into a Session snapshot.
