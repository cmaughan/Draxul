...
- **SatView propagation service**: SatView builds a module-local SGP4 propagation model from the current catalog using the pinned Vallado/CelesTrak AIAA-2006-6753 reference implementation. The host propagates current TEME positions, approximate Earth-fixed positions, and capped preview track samples on a one-second cadence independent of the render frame tick; reference tests verify the Vallado satellite 00005 case. The renderer consumes those propagated TEME tracks and markers through one dynamic line-list vertex stream instead of drawing synthetic rings.
