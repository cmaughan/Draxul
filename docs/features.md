...
| SatView | `--host satview` | Optional 3D satellite-overview host with an interactive texture-mapped Earth, approximate live day/night lighting, async CelesTrak GP catalog fetch/cache status, SGP4 propagation status, catalog-derived orbit tracks and satellite markers, orbit-class or object-type coloring, all-sampled or selected-only path display, track/marker LOD controls, click selection, an ImGui filter/details panel, no-roll quaternion left-drag orbit controls, orbit-aware mouse-wheel zoom, reset camera (`Home`), catalog refresh (`R`), panel toggle (`F1`), and time-speed controls (`Space`, `[`, `]`) |
...
- **SatView Earth pass**: The optional SatView module renders a texture-mapped Earth through the shared 3D render-pass path on Vulkan and Metal, using staged 8k equirectangular day, night, and cloud maps with approximate day/night lighting, atmospheric rim light, catalog-derived orbit tracks, satellite point markers, no-roll quaternion orbit controls that stay just shy of the pole singularity, and an orbit-aware zoom ceiling that expands to the visible satellite/orbit radius. It is launched with `--host satview`.
...
- **SatView interaction panel**: SatView owns a pane-local ImGui panel toggled by `F1`, with pause/resume, reset camera, catalog refresh, time-speed, color mode (`Orbit Class` or `Object Type`), path display (`All Sampled` or `Selected Only`), track/marker LOD controls, search, orbit-class, optional object type/classification, source-label, and element-age filters. Click selection projects visible satellite markers to screen space, highlights the nearest selected object in the dynamic line stream, samples the selected object's path even when it is outside the general path cap, and reports details including NORAD id, international designator, orbit class, normalized object kind, period, element age, altitude, and speed.
