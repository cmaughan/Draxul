...
- **MegaCity code source modes**: The City Build UI can build the semantic city directly from a completed Tree-sitter scan or directly from a Graphify graph JSON. The Tree-sitter mode no longer writes a SQLite city snapshot before layout; SQLite citydb remains only for focused persistence/debug tests. Graphify mode defaults to `graphify-out/graph.json` and still feeds the same semantic-city builder, module filtering, dependency routing, labels, and metrics pipeline
