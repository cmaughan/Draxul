...
| Rezonality | `--plugin dev.draxul.rezonality` at launch or on pane/tab commands | Fault-tolerant Vulkan/Metal live graphics viewer ported from VkLive. Direct launch creates and focuses a server-topology plugin tab while preserving terminal access. It watches external edits, compiles complete GLSL candidates off the UI thread, renders named surfaces plus OBJ/glTF models with cameras, PBR material textures, and HDR environments, and retains the last valid GPU generation when a shader or asset edit fails |
...
| `DRAXUL_ENABLE_REZONALITY` | ON | Builds and stages the `dev.draxul.rezonality` DLL/dylib, preserved platform shader compiler, bundled simple/default/waves/deferred/disc/PBR projects, live-edit runtime, Assimp model loader, and Vulkan/Metal multipass renderer |
