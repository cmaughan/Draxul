...
- Feature, bug and refactor review prompts require architecture mapping, explicit subsystem delegation to at most four read-only workers without nested delegation, representative flow/caller/test investigation across all major areas, coordinator verification and a final cross-component pass. Astra enables subagents and pins their defaults to the selected model at high effort, with a four-worker concurrency limit; nonce preflight disables delegation. Reports have no finding-count cap and must disclose investigation coverage and gaps. Unsupported delegation falls back to sequential review. Consensus verifies accepted claims against current code and separates unresolved leads from Kanban tasks
