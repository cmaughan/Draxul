...
| Rezonality | `--plugin dev.draxul.rezonality` at launch or on pane/tab commands | Fault-tolerant Vulkan/Metal live shader viewer ported from VkLive. Direct launch creates and focuses a server-topology plugin tab while preserving terminal access. It loads bundled single/multipass projects or a configured directory, watches external edits, compiles complete GLSL candidates off the UI thread, renders named color/float/depth/texture surfaces with frame-safe animated common uniforms, and retains the last valid GPU generation when an edit fails |
...
| `DRAXUL_ENABLE_REZONALITY` | ON | Builds and stages the `dev.draxul.rezonality` DLL/dylib, preserved platform shader compiler, bundled simple/default/waves/deferred/disc projects, live-edit runtime, and Vulkan/Metal multipass renderer |
