...
- **SatView propagation service**: SatView builds a module-local SGP4 propagation model from the current catalog using the pinned Vallado/CelesTrak AIAA-2006-6753 reference implementation. Satellite positions are propagated on a background worker and published through a non-blocking triple-buffer snapshot handoff so rendering can read the latest complete state without waiting on the worker; orbit track samples update on a slower cadence while marker positions refresh frequently and render-side interpolation smooths visible motion between worker ticks. Reference tests verify the Vallado satellite 00005 case. The renderer consumes those propagated TEME tracks and markers through one dynamic line-list vertex stream instead of drawing synthetic rings.
