...
- **SatView Earth pass**: The optional SatView module renders a texture-mapped Earth through the shared 3D render-pass path on Vulkan and Metal, using staged 8k equirectangular day, night, and cloud maps with approximate day/night lighting, atmospheric rim light, catalog-derived orbit tracks, satellite point markers, quaternion orbit controls that avoid pole flips, and an orbit-aware zoom ceiling that expands to the visible satellite/orbit radius. It is launched with `--host satview`.
