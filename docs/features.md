...
- **MegaCity recursive folder districts**: Each populated source folder gets a local building spiral; MegaCity recursively composes those leaf districts into padded, labeled parent-folder spirals, so nested projects remain visibly contained by ancestors such as `plugins/<product>` and the overall `plugins` district. Conventional `include/` and `src/` trees collapse into their owning project, grouping headers and implementations into the same module spiral without boundaries beneath those source roots. Empty branches are omitted, and every leaf-module plus folder boundary draws a thin path-colored outline above the shared road layer
...
- **MegaCity semantic snapshot**: The City Build UI builds the semantic city from the same neutral `CodeSemanticSnapshot` used by BioView. Tree-sitter scanner output is first projected into repository/module/file/type/function/method/field/reference nodes, then the city builder derives direct source-folder identity and applies city-specific roles, building metrics, function layers, dependency routing, and recursive folder composition before layout. Ancestor folders become layout-only containing districts rather than fake semantic modules. The shared semantic project-module identity remains unchanged for BioView tissues and module filtering
