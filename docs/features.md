...
- `launch_agent` is available in the command palette and unbound by default. It prompts for a local command (for example `codex`), opens a shell split in the active Space, starts that command, and records durable kind/display/instance identity on the pane. Session restore starts a fresh command in the restored pane; process and native conversation resume are deliberately not implied.
