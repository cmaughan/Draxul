...
| SatView | `--host satview` | Optional 3D satellite-overview host with an interactive texture-mapped Earth, approximate live day/night lighting, a loaded sample GP satellite catalog count in the pane status, left-drag orbit controls, mouse-wheel zoom, and time-speed controls (`Space`, `[`, `]`) |
...
- **SatView catalog parser**: SatView now includes module-local satellite catalog records, a CelesTrak GP JSON parser for OMM keyword fields, orbit-class derivation from mean motion/eccentricity, and a synthetic offline sample catalog loaded into the host status path as the first Phase 2 data slice.
