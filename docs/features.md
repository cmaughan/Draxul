...
| PCBView | `--plugin dev.draxul.pcbview` at launch or on pane/tab commands | Native NanoVG PCB canvas backed by strict millimetre JSON: a zoomable/pannable ten-layer 100 mm board with 500 1.5/0.5 mm through holes, degree-2 connectivity, bounded layer-colored autorouting, global via clearances, selectable routed/failed connections, back-to-front compositing, and plugin-owned ImGui controls |
