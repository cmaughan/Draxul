...
| Score | `--host score [--source <file.musicxml/.mxl>]` | Music score viewer host, in development ([plans/scoreview.md](../plans/scoreview.md)). Phase 0 renders a placeholder engraved page (A4 page with drop shadow, grand staff at SMuFL engraving proportions, brace, measure/final barlines) via the shared NanoVG pass. `--source` is accepted and echoed in the status bar; MusicXML import and Verovio-based engraving arrive in later phases. Gated by `DRAXUL_ENABLE_SCOREVIEW` (default ON) |
