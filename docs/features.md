...
- `launch_agent` is available in the command palette and unbound by default. It opens a profile picker with built-in Codex and Claude entries plus structured `[agents.profiles.<id>]` configuration. Managed agents launch directly as the PTY/ConPTY child using executable-plus-argv data, receive Draxul Session/Space/tab/pane/instance routing environment variables, and record durable kind/display/instance identity on the pane. Legacy saved shell-command agents retain their compatibility path. Codex and Claude can additionally install opt-in, versioned `SessionStart` hooks with `draxul integration install codex|claude`; each hook reports the official native conversation ID back to its owning pane. Bare `draxul integration status` inspects both integrations without modifying configuration.
...
- Codex and Claude started manually inside ordinary shell panes are discovered best-effort from local process evidence. Unix uses the PTY foreground process group; Windows uses the ConPTY descendant tree and reports lower confidence because ConPTY has no portable foreground-group query. Direct executables, structured Node/Bun/npm wrappers, and `DRAXUL_AGENT=codex|claude` hints are recognized. Discovered rows are immediate but transient, survive six failed probes to avoid flicker, can be dismissed with `clear_agent_identity`, are never checkpointed, and can never acquire a resumable native-session reference through inference.
- Normal desktop Sessions expose a same-user local control endpoint: an authenticated named pipe on Windows or owner-only Unix-domain socket on macOS/Linux. `draxul space list/get`, `agent list/get/explain`, and `pane read` provide concise or `--json` read-only inspection by Session ID. Requests are bounded and versioned; transport work is isolated from the application model, and all Space, agent, pane, and host reads run on Draxul's main thread.
