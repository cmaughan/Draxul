...
- **SatView Earth pass**: The optional SatView module renders a texture-mapped Earth through the shared 3D render-pass path on Vulkan and Metal, using staged 8k equirectangular day, night, and cloud maps with approximate day/night lighting, atmospheric rim light, Greenwich-sidereal-aligned TEME orbit tracks and satellite point markers, an EasyRender-derived camera/manipulator with smoothed quaternion orbit and dolly controls, a per-frame look-at view matrix, and an orbit-aware dolly ceiling that expands to the visible satellite/orbit radius. It is launched with `--host satview`.
