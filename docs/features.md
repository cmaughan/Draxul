...
- **SatView interaction panel**: SatView owns a pane-local ImGui panel toggled by `F1`, with pause/resume, reset camera, catalog refresh, time-speed, search, orbit-class, optional object type/classification, source-label, and element-age filters. Click selection projects visible satellite markers to screen space, highlights the nearest selected object in the dynamic line stream, and reports details including NORAD id, international designator, orbit class, period, element age, altitude, and speed.
