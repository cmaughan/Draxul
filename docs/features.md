...
| PCBView | `--plugin dev.draxul.pcbview` at launch or on pane/tab commands | Native NanoVG PCB canvas backed by strict millimetre JSON: a zoomable/pannable ten-layer 100 mm board with 500 1.5/0.5 mm through holes, degree-2 connectivity, centre-constrained adaptive per-layer and 3D A* autorouting with arbitrary clear via transitions plus deterministic rip-up/reroute, global via clearances, selectable routed/failed connections, failed-route focus that fades unrelated pads and wires, back-to-front compositing, and plugin-owned ImGui controls |
...
The render-test harness can attach a pre-capture plugin-reload step to a
scenario. It first waits for the original generation to render and settle,
atomically publishes a complete replacement package, reloads the matching local
cohort, then requires a newer rendered frame before capture. The publication
marker and temporary generation are restored afterward, so repeated Vulkan and
Metal smoke runs start from the same installed generation.
...
On Vulkan, Draxul enables buffer-device-address support when the selected
device advertises it, and enables the acceleration-structure and ray-pipeline
extension/feature set only when the complete set is available. This lets a
trusted plugin record native ray work through the existing borrowed frame
without making ray tracing a startup requirement for Draxul or raster plugins.
Unsupported devices keep the normal raster path and let the plugin publish an
...
- **MegaCity performance preview and coverage modes**: The Codebase Analysis panel exposes saved top-level `Perf`, `Coverage`, `LCOV Coverage`, and `Perf Log Scale` controls. `Perf` blends flat-color buildings toward a green-to-red heat palette per semantic building layer using smoothed live timing heat, while `Coverage` forces any touched/matched function layer to full heat so executed code lights up clearly. `LCOV Coverage` can import an externally produced static `lcov` tracefile from `db/coverage.lcov` or `build/coverage.lcov`; Draxul no longer builds or exports instrumented coverage reports itself. The debug panel shows LCOV-specific diagnostics, and the building tooltip reports per-function status. `Perf Log Scale` applies a visual logarithmic boost to low heat values without changing the underlying timing data.
...
- When SatView is enabled, the core App integration target stages its real plugin package and checks that pane preferences survive close/reopen and native reload; App smoke tests also inspect the read-only chrome layout after weather config reload and drive a real Neovim host through font replacement.
...
- `do validate debug` is the explicit complete local gate: one serialized app/full-test build followed by bounded smoke, the default core render comparisons (or repeated `--render <scenario>` selections), and the complete unit CTest inventory from the same cache. It does not reconfigure between stages. Successful commands emit concise status, while failures retain their full logs under the selected build tree and the final summary distinguishes build, startup, snapshot, product-test, and validation-environment failures
- On Windows, `do test` and `do validate` inspect the default live server after the selected cache is built. They leave an unchanged same-cache helper or a server from another build cache running. If that server owns a stale `draxul-server.exe` which the selected app must replace, the workflow exits as a validation-environment failure before starting tests and reports its exact PID, runtime directory, attached-client count, checkpoint health, and explicit shutdown command; it never stops the server automatically. Detached-server integration cases stage a private app/helper beside their unique runtime, preventing an unrelated default server from locking the test helper
...
- The repo-scoped `$draxul-review` skill runs isolated, read-only multi-AI reviews through installed Codex, Claude, Agy/Gemini, and Grok CLIs. Its default review and preflight panel uses OpenAI GPT-6 Sol (`gpt-6-sol`) and Claude Opus 5.5 (`claude-opus-5-5`), both at high effort; synthesis also defaults to GPT-6 Sol at high effort. Explicit panels reject duplicate companies, and `--all` includes Google and xAI. When a synthesis prompt requests Kanban work items, the trusted parent runner validates the returned card paths/content and atomically creates them under `kanban/pending/`; providers never receive repository write access
- Codex/Claude reviews and synthesis explicitly use high reasoning effort. Reviews first run the installed `repomix` CLI over a shared frozen snapshot, including initialized submodules and dirty/non-ignored untracked files. Missing submodules fail explicitly. Repomix generates one uncompressed XML file with directory structure, paths and line numbers; binary/security filtering remains enabled. Previous review archives and `repomix-output.*` files are excluded. Each provider receives a private copy of that same packed file and produces one report in one coordinating run, without batches or segment receipts. The archived input, SHA-256 provenance and Repomix diagnostics make inputs traceable without claiming exhaustive model coverage. `review --plan-only` generates the packed input without provider calls; `--timeout` applies to each review, and separately to Repomix preparation
- Feature, bug and refactor review prompts require architecture mapping, explicit subsystem delegation to at most four read-only workers without nested delegation, representative flow/caller/test investigation across all major areas, coordinator verification and a final cross-component pass. Codex enables subagents and pins their defaults to the selected model at high effort, with a four-worker concurrency limit; nonce preflight disables delegation. Reports have no finding-count cap and must disclose investigation coverage and gaps. Unsupported delegation falls back to sequential review. Consensus verifies accepted claims against current code and separates unresolved leads from Kanban tasks
- Codex review calls use structured JSON events to separate reviewed source, tool output, and final reports from runtime diagnostics. Only stderr and explicit error events feed runtime-failure detection and the Windows sandbox retry check, so quoted error strings do not fail a completed review or trigger a fallback
- Review and synthesis calls default to a 90-minute timeout (`--timeout` overrides it). The review skill schedules status reports every 10 minutes through the host scheduler, following the exact review and consensus manifests until completion, verifying output/task files, and disabling the monitor after the final report
- Review reports and diagnostics are saved immediately as each provider finishes, with incremental running manifests. Consensus runs automatically after the panel completes, using that exact run’s successful reports and the matching saved consensus prompt; partial panels identify missing reviewers. `--no-consensus` opts out, `--consensus-prompt` overrides the prompt, and `--summarizer` selects the synthesizer (default: GPT-6 Sol at high effort). Review-mode consensus and standalone `summarize` create validated Kanban cards by default; `--no-kanban` opts out. `--no-consensus` skips synthesis and card creation together. Consensus failure preserves completed reviews; its state and run link are recorded separately in the review manifest. Synthesis responses are archived before card publication. Immutable artifacts live under `plans/reviews/runs/`; separate `*-latest.manifest.json` and `*-latest.summary.manifest.json` files keep review and consensus status distinct
- `$draxul-preflight` checks installation, authentication, requested-model access, and a nonce-based live response before review work. Agy is preferred for Google and falls back to Gemini when unavailable, with the fallback recorded in the run manifest. Reviewers and synthesis have a 90-minute default timeout and emit flushed lifecycle events. Real Codex, Claude, and Grok review/synthesis sessions persist in their normal provider stores for TokenFu tool-call and token accounting, while nonce-only preflight sessions remain ephemeral and cross-session memory stays disabled. A Codex review that hits native Windows sandbox error 1312 retries once without the broken OS sandbox inside its disposable repository snapshot, retains the fixed review-only contract and original total timeout, and records the fallback in the manifest
