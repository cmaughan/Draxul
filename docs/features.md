...
- **SatView propagation service**: SatView builds a module-local SGP4 propagation model from the current catalog using the pinned Vallado/CelesTrak AIAA-2006-6753 reference implementation. Satellite positions are propagated on a background worker and published through a non-blocking triple-buffer snapshot handoff so rendering can read the latest complete state without waiting on the worker; marker positions refresh frequently and render-side interpolation smooths visible motion between worker ticks. Orbit tracks are cached and rebuilt only when the catalog or geometry-affecting track settings change, avoiding periodic CPU regeneration and GPU line-buffer uploads. Reference tests verify the Vallado satellite 00005 case. The renderer consumes propagated TEME tracks through a track line-list buffer and satellite markers through a separate marker instance buffer that shader-expands camera-facing crosses, avoiding per-frame marker vertex rebuilds during camera-only redraws.
- **SatView interaction panel**: SatView owns a pane-local ImGui panel toggled by `F1`, with a `Globe`/`Map` view switch, an Earth/Moon globe POV selector, pause/resume, reset camera/map center, catalog/cloud refresh, time-speed, cloud visibility, realistic-cloud source selection, atmosphere and Moon visibility, color mode (`Name Prefix`, `Orbit Class`, or `Object Type`), path display (`All Sampled` or `Selected Only`), track/marker LOD controls, search, orbit-class, optional object type/classification, source-label, and element-age filters. Switching POV preserves camera orientation and apparent body size, then continuously follows the selected body; camera reset and dolly limits scale to that body's real radius. Object filters sit directly above the object tree, which immediately removes non-matching entries and reports the filtered count against the full catalog count. In map mode, left-drag pans the spherical projection center, the existing horizontal and vertical camera keys pan it continuously, and `Home` restores Greenwich/equator as the center; a click without a drag still selects the nearest marker. The catalog area reports cloud cache/fetch status and the required EUMETSAT attribution. Name-prefix mode uses the same stable bright pastel for every object grouped under a tree prefix, so constellations such as STARLINK remain visually consistent. Click selection projects visible satellite markers through the active globe or map projection, while the object tree groups all propagated objects by orbit class and name prefix for direct selection. Selected objects remain force-visible, highlight in the dynamic line stream, sample their path even when outside the general path cap, and report details including NORAD id, international designator, orbit class, normalized object kind, period, element age, WGS-84 latitude/longitude, altitude, and speed.
