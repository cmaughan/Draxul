...
| SatView | `--host satview` | Optional satellite-overview host with switchable interactive 3D globe and full-screen 2D equirectangular map views, Earth/Moon POV selection in both views, date-aware day/night lighting, a real-scale ephemeris-driven Moon with an 8k NASA LRO texture and analytical orbit track, a ray-marched Rayleigh/Mie atmosphere, an elevated cloud shell using bundled clouds by default with an optional asynchronously cached near-real-time source, async CelesTrak GP catalog fetch/cache status, SGP4 propagation status, catalog-derived orbit tracks and satellite markers, stable bright name-prefix coloring plus orbit-class and object-type alternatives, all-sampled or selected-only path display, track/marker LOD controls, click and tree selection, an ImGui filter/details panel with a live simulation-clock readout in the user's local timezone, a `Real Time` action that restores the current system time at `1x`, smoothed quaternion left-drag orbit controls, Ctrl+drag and mouse-wheel dolly, MegaCity-style keyboard orbit/dolly controls, reset camera (`Home`), data refresh (`Ctrl+R`), panel toggle (`F1`), and time-speed controls (`Space`, `[`, `]`) |
...
- **SatView Earth pass**: The optional SatView module renders a texture-mapped Earth through the shared 3D render-pass path on Vulkan and Metal, using staged 8k equirectangular day, night, and cloud maps with a UTC date-aware solar ephemeris, seasonal declination, Greenwich-sidereal-aligned TEME orbit tracks and satellite point markers, a quaternion-owned camera/manipulator with continuous pole-crossing orbit and dolly controls, a per-frame look-at view matrix, GPU marker billboards derived from the same camera quaternion, and an orbit-aware dolly ceiling that expands to the visible satellite/orbit radius. Globe mode also renders the Moon at its analytical geocentric position, instantaneous Earth distance, and real radius using NASA's 8k LRO WAC color mosaic. The Moon has its own atmosphere-free diffuse pass, phase-dependent Earthshine, a stable IAU-pole-aligned tidal orientation, a visibility toggle, and an expanded camera ceiling that can frame the full Earth-Moon separation. Its independently toggleable pale orbit track spans one sidereal lunar cycle, uses the shared track-sample control, and remains stable until simulation time leaves the sampled half-month window; it is omitted from Moon map mode where a self-relative track has no useful projection. The same pass can switch to a six-vertex full-screen equirectangular map targeting either body. Earth POV projects markers to their current Earth subpoints and uses each orbit sample's own Earth-fixed position, producing temporal ground tracks rather than rotating an entire inertial orbit ring at one instant. Earth ground tracks remain open between their first and last sampled times, avoiding a false closing segment. Moon POV displays the lunar mosaic and projects every satellite and orbit point by its Moon-centered line of sight in the tidally locked lunar frame, so the Earth-orbiting catalog gathers around the Earth-facing hemisphere. Paired line endpoints and wrapped draw copies clip cleanly at both map edges without introducing internal path gaps. The map uses a rotated equirectangular projection whose center longitude/latitude can be changed without rebuilding or re-uploading the cached orbit buffer; surface sampling, solar lighting, markers, tracks, wrapping, and picking share the selected body's transform. Existing filters, colors, path modes, interpolation, selection, day/night lighting, and cloud source controls continue to apply. A separate atmosphere shell jointly ray-marches Rayleigh and Mie view/sunlight optical depth through 8 km molecular and 1.2 km aerosol scale heights, including wavelength-dependent extinction, Rayleigh and forward Henyey-Greenstein phase scattering, planet shadow, and premultiplied depth-aware composition. Clouds render on their own premultiplied shell about 9.5 km above the surface, giving them geometric separation and independent day/night lighting instead of painting them into the opaque Earth surface; in Earth map mode the selected cloud texture is composited directly over the flat Earth map. A background service downloads the latest 8k Live Cloud Maps EUMETSAT-derived texture and caches it for the source's three-hour update cadence. Bundled and live cloud maps keep separate GPU bindings: the bundled map is selected by default, `Realistic clouds` opts into the live map, and the master `Clouds` switch can disable the shell without discarding either texture. The controls show the advancing simulation date/time in the user's system timezone with its UTC offset. The `Real Time` button atomically unpauses, sets simulation time to the current system instant, resets speed to `1x`, and refreshes propagated positions. It is launched with `--host satview`.
- **SatView catalog service**: SatView includes module-local satellite catalog records, a CelesTrak GP JSON parser for OMM keyword fields, orbit-class derivation from mean motion/eccentricity, an async CelesTrak `active` fetch path by default, group-specific cache files under the platform cache directory, a two-hour refresh guard aligned with CelesTrak GP cadence, synthetic offline sample fallback, and status text that distinguishes live, cached, sample, refreshing, and failed catalog states.
- **SatView propagation service**: SatView builds a module-local SGP4 propagation model from the current catalog using the pinned Vallado/CelesTrak AIAA-2006-6753 reference implementation. Satellite positions are divided into deterministic index ranges and propagated by a persistent worker pool; multi-satellite track generation uses the same pool, while small selected-only path jobs stay local to avoid dispatch overhead. Automatic concurrency leaves two hardware threads available and is capped at 16 after profiling the continuous position workload. Completed states and tracks retain catalog order and are published through a non-blocking triple-buffer snapshot handoff so rendering can read the latest complete state without waiting on simulation work; marker positions refresh frequently and render-side interpolation smooths visible motion between worker ticks. The shared render timestamp is bounded to the published position bracket, keeping marker positions, map longitude, Earth rotation, picking, lighting, and ephemerides synchronized even while a selection triggers an expensive track rebuild. General orbit tracks are cached and rebuilt only when the catalog or geometry-affecting track settings change. A selected track alone is resampled after its marker moves one quarter of the sampled time horizon from the track center, keeping the marker inside a sliding past/future window without periodically repropagating every displayed orbit. A default-off experimental `Refresh paths every step` control bypasses that cache and regenerates all configured paths on every worker simulation step, with a potentially substantial CPU and GPU-upload cost at high track/sample counts. Reference tests verify the Vallado satellite 00005 case. The renderer consumes propagated TEME tracks through a track line-list buffer and satellite markers through a separate marker instance buffer that shader-expands camera-facing crosses, avoiding per-frame marker vertex rebuilds during camera-only redraws.
- **SatView interaction panel**: SatView owns a pane-local ImGui panel toggled by `F1`, with a `Globe`/`Map` view switch, an Earth/Moon POV selector, pause/resume, reset camera/map center, catalog/cloud refresh, time-speed, cloud visibility, realistic-cloud source selection, atmosphere and Moon visibility, color mode (`Name Prefix`, `Orbit Class`, or `Object Type`), satellite display (`Tracks + Satellites`, `Tracks Only`, or `Satellites Only`), path display (`All Sampled` or `Selected Only`), track/marker LOD controls, search, orbit-class, optional object type/classification, source-label, and element-age filters. Durable panel settings are merged into the `[satview]` user-config table and restored on startup; transient simulation time, pause, selection, and camera placement are not persisted. Track generation defaults to 1,024 objects, ranges from one to the full propagated catalog size, and supports up to 256 samples per track. A selected object consumes one track slot and replaces the last general track when necessary, so a track count of one shows only the selection. In globe mode, switching POV preserves camera orientation and apparent body size, then continuously follows the selected body; camera reset and dolly limits scale to that body's real radius. Moon POV permits near-surface dolly using a clearance-derived near plane, while wheel and keyboard zoom share the camera's active body-specific distance limits so switching POV cannot force an Earth-scale bounce. In map mode, the same selector changes the mapped surface and satellite reference body. Left-drag pans the spherical projection center, the existing horizontal and vertical camera keys pan it continuously, and `Home` restores zero longitude/equator as the center; a click without a drag still selects the nearest visible marker. Object filters sit directly above the object tree, which immediately removes non-matching entries and reports the filtered count against the full catalog count. The catalog area reports cloud cache/fetch status and the required EUMETSAT attribution. Name-prefix mode uses the same stable bright pastel for every object grouped under a tree prefix, so constellations such as STARLINK remain visually consistent. Click selection projects visible satellite markers through the active globe or map projection, while the object tree groups all propagated objects by orbit class and name prefix for direct selection. Selected objects remain force-visible, highlight in the dynamic line stream, sample their path within the path cap even when outside its general catalog range, and report details including NORAD id, international designator, orbit class, normalized object kind, period, element age, WGS-84 latitude/longitude, altitude, and speed.
...
- **SatView camera/map**: In globe mode, left-drag orbits and `Ctrl` + left-drag/mouse wheel dollies; `A/D`, left/right arrows, or `Q/E` orbit horizontally, `W/S`, up/down arrows, or `T/G` orbit vertically, and `R/F` dolly in/out. In map mode, left-drag pans the projection while the same horizontal/vertical keys move its center longitude/latitude.
