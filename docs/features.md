...
| SatView | `--host satview` | Optional 3D satellite-overview host with an interactive texture-mapped Earth, approximate live day/night lighting, async CelesTrak GP catalog fetch/cache status, SGP4 propagation status, catalog-derived orbit tracks and satellite markers, orbit-class or object-type coloring, all-sampled or selected-only path display, track/marker LOD controls, click and tree selection, an ImGui filter/details panel, smoothed quaternion left-drag orbit controls, Ctrl+drag and mouse-wheel dolly, reset camera (`Home`), catalog refresh (`R`), panel toggle (`F1`), and time-speed controls (`Space`, `[`, `]`) |
...
- **SatView Earth pass**: The optional SatView module renders a texture-mapped Earth through the shared 3D render-pass path on Vulkan and Metal, using staged 8k equirectangular day, night, and cloud maps with approximate day/night lighting, atmospheric rim light, Greenwich-sidereal-aligned TEME orbit tracks and satellite point markers, a quaternion-owned camera/manipulator with continuous pole-crossing orbit and dolly controls, a per-frame look-at view matrix, GPU marker billboards derived from the same camera quaternion, and an orbit-aware dolly ceiling that expands to the visible satellite/orbit radius. It is launched with `--host satview`.
