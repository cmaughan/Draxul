...
- **SatView Sun body**: SatView places the Sun at its date-aware geocentric direction and Earth-Sun distance with the nominal 695,700 km radius, so it has the expected roughly half-degree apparent diameter from Earth. A dedicated emissive HDR pass renders the 4k Solar System Scope texture with limb darkening, an IAU 7.25-degree axis orientation, and a rigid 25.38-day sidereal texture rotation. The persisted `Sun` visibility control and Earth/Moon/Sun POV selector work in globe and map modes; Sun map markers and paths use the same rotating solar body frame as picking. The Sun has no orbit track because SatView does not visualize the Sun's galactic orbit.
