...
| `plugins/rezonality/` | `DRAXUL_ENABLE_REZONALITY` | Submodule → [draxul-rezonality](https://github.com/cmaughan/draxul-rezonality). Fault-tolerant live scene/shader product ported from VkLive; owns its watched project loader, compiler tools, immutable multipass SPIR-V candidates, named GPU surfaces/textures, Vulkan/Metal passes, examples, diagnostics, tests, and remaining product slices |
