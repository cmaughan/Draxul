...
| `libs/draxul-client/` | Experimental singleton discovery/launch/status plus renderer-free remote-terminal and revisioned topology polling projections, scrollback-page client, attach-latency metric, and headless probe API |
| `libs/draxul-server/` | Headless server kernel, authoritative revisioned/idempotent topology service, deterministic fake terminal plus opt-in real server-owned shell/runtime configuration, semantic scrollback, controller leases, sanitized transport metrics, bounded per-client terminal event queues, and serialized control event loop; deliberately has no window, renderer, host, or product dependency |
