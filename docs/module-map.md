...
| `libs/draxul-client/` | Singleton discovery/launch/status/shutdown plus renderer-free remote-terminal, topology, and sanitized agent polling projections consumed by the app's live Space/tab/pane/Agents adapters, scrollback-page client, attach-latency metric, and headless probe API |
