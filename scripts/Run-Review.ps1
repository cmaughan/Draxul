param(
    ...
    [string]$PromptStem = "review",
    [string]$ReviewBasename = "review-latest",
    [string]$ConsensusBasename = ""
)
...
if (-not $ConsensusBasename) {
    if ($PromptStem -eq "review") {
        $ConsensusBasename = "review-consensus"
    } elseif ($PromptStem -eq "review_bugs") {
        $ConsensusBasename = "review-bugs-consensus"
    } else {
        $ConsensusBasename = $PromptStem.Replace("_", "-") + "-consensus"
    }
}

$reviewPromptPath = Join-Path $repoRoot ("plans\prompts\" + $PromptStem + ".md")
$consensusPromptName = if ($PromptStem -eq "review") { "consensus_review.md" } else { "consensus_" + $PromptStem + ".md" }
$consensusPromptPath = Join-Path $repoRoot ("plans\prompts\" + $consensusPromptName)
$reviewsDir = Join-Path $repoRoot "plans\reviews"
$codexReviewPath = Join-Path $reviewsDir ($ReviewBasename + ".gpt.md")
$geminiReviewPath = Join-Path $reviewsDir ($ReviewBasename + ".gemini.md")
$claudeReviewPath = Join-Path $reviewsDir ($ReviewBasename + ".claude.md")
$consensusReviewPath = Join-Path $reviewsDir ($ConsensusBasename + ".md")
$codexRunLogPath = Join-Path $reviewsDir ($ReviewBasename + ".gpt.run.log")
$geminiRunLogPath = Join-Path $reviewsDir ($ReviewBasename + ".gemini.agy.log")
$geminiStdIoLogPath = Join-Path $reviewsDir ($ReviewBasename + ".gemini.stdio.log")
$claudeRunJsonPath = Join-Path $reviewsDir ($ReviewBasename + ".claude.run.json")
$consensusCodexMessagePath = Join-Path $reviewsDir ($ConsensusBasename + ".codex-message.md")
$consensusRunLogPath = Join-Path $reviewsDir ($ConsensusBasename + ".codex.run.log")
$codexReviewModel = "gpt-5.5"
$claudeReviewModel = "opus"
...
"@
    ...
    $result = Invoke-NativeCommand -LogPath $codexRunLogPath -ScriptBlock {
        $codexReviewPrompt |
            & $codexCommand exec --skip-git-repo-check -C $repoRoot --model $codexReviewModel --sandbox danger-full-access -o $codexReviewPath -
    }
...
"@
    ...
    $claudeJson = $claudePrompt |
        claude -p --model $claudeReviewModel --output-format json --permission-mode bypassPermissions --allowedTools Read Grep Glob Bash |
        Out-String
    $exitCode = $LASTEXITCODE
...
function Run-ConsensusReview {
    ...
    $result = Invoke-NativeCommand -LogPath $consensusRunLogPath -ScriptBlock {
        $consensusPrompt |
            & $codexCommand exec --skip-git-repo-check -C $repoRoot --model $codexReviewModel --sandbox danger-full-access -o $consensusCodexMessagePath -
    }
    $exitCode = $result.ExitCode
    if ($exitCode -ne 0) {
